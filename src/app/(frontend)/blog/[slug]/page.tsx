    ...
    <article className="pt-16 pb-20">
      ...
      <div className="border-y border-border bg-card/55">
        <div className="container flex flex-wrap items-center justify-between gap-4 py-4 text-sm text-muted-foreground">
          <Link
            className="inline-flex items-center gap-2 font-medium text-foreground hover:text-[#E8501A]"
            href="/blog"
          >
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
            {blog.publishedAt && (
              <span className="inline-flex items-center gap-2">
                <CalendarDays className="size-4 text-[#E8501A]" />
                {formatDateTime(blog.publishedAt)}
              </span>
            )}
            <span className="inline-flex items-center gap-2">
              <UserRound className="size-4 text-[#E8501A]" />
              {authorLabel}
            </span>
          </div>
        </div>
      ...
      <div className="container grid gap-12 pt-14 lg:grid-cols-[minmax(0,1fr)_18rem] xl:grid-cols-[minmax(0,1fr)_21rem]">
        <div className="min-w-0">
          <div className="mb-10 border-l-2 border-[#E8501A] pl-5 md:pl-7">
            ...
            {categories.length > 0 && (
              <div className="rounded-lg border border-border bg-background p-5">
                <div className="mb-4 flex items-center gap-2">
                  <Compass className="size-4 text-[#E8501A]" />
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((category) => {
                    ...
                    return category.slug ? (
                      <Link
                        className="rounded-full border border-border bg-card px-3 py-1.5 text-sm text-muted-foreground transition-colors hover:border-[#E8501A] hover:text-[#E8501A]"
